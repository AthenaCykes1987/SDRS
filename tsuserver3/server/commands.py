...
def ooc_cmd_login(client, arg):
    ...
    try:
        client.auth_mod(arg) and client.auth_rpgm1(arg) and client.auth_rpgm2(arg) and client.auth_rpgm3(arg)
    except ClientError:
        raise
    if client.auth_mod:
        client.send_host_message('Logged in as moderator')
        logger.log_server('Logged in as moderator.', client)
    else:
        client.send_host_message('Logged in as GM')
        logger.log_server('logged in as GM', client)
    client.in_rp = False
...
def ooc_cmd_play(client, arg):
    if not client.is_mod and not client.is_rpgm1 and not client.is_rpgm2 and not client.is_rpgm3:
        raise ClientError('You must be authorized to do that.')
    if len(arg) == 0:
...
def ooc_cmd_rpmode(p_client, arg):
    if not p_client.is_mod and not client.is_rpgm1 and not client.is_rpgm2 and not client.is_rpgm3:
        raise ClientError('You must be authorized to do that.')
    if len(arg) == 0:
